...
@fixtures.command('create-record')
@click.option('--file', '-f',
...
@with_appcontext
def create_record(file, ana, role, user, owner):
    """
    Create a new deposit through the CLI.
    Usage:
        cap record create --data DATAFILE --role ROLE --user USER --owner OWNER
    """
    try:
